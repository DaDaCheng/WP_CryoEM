...
def prepare_test_dataset_X(sinogram_noise,A,n_neighbors,walk_len,batch_size,N,iter_index,directed):
    ...
    if iter_index==-1:
        ...
        A=A.astype(np.int)
        A_noise_d=neighbotFromX(sinogram_noise.T, n_neighbors=n_neighbors*2)
        A_noise_d=A_noise_d.astype(np.int)
        if directed:
            A_noise_d=(A_noise_d-A)
            pos_edge_index=edge_index[A.reshape(-1)==1]
        ...
        if iter_index==0:
            ...
            A_noise_dm=neighbotFromX(sinogram_noise.T, n_neighbors=n_neighbors*iter_index)
            A_noise_d=neighbotFromX(sinogram_noise.T, n_neighbors=n_neighbors*(iter_index+1))
            A_noise_d=A_noise_d.astype(np.int)
            A_noise_dm=A_noise_dm.astype(np.int)
            if directed:
                A_noise_d=A_noise_d-A_noise_dm
